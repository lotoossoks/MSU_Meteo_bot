...
cursor = conn.cursor()
cursor.execute('SELECT name_on_disk FROM devices')
results = cursor.fetchall()
list_devices = [result[0] for result in results]
cursor.execute('SELECT name_on_disk FROM complexes')
results = cursor.fetchall()
list_complexes = [result[0] for result in results]
...
def upload_all_files_from_disk(message):
    for name_folder in list_devices:
        for name_file in os.listdir(f'{disk}/{name_folder}'):
            if name_file.endswith('.csv'):
                shutil.copy(f'{disk}/{name_folder}/{name_file}', f'{main_path}/{name_folder}/{name_file}')

    for name_folder in os.listdir(f'{main_path}'):
        for name_file in os.listdir(f'{main_path}/{name_folder}'):
            if name_file.endswith('.csv'):
                preprocessing_one_file(name_folder, name_file)


def preprocessing_one_file(device, name_file):
    df = pd.read_csv(f'{main_path}/{device}/{name_file}', sep=None, engine='python')
    f = load_json('config_devices.json')
    # Обновление config_devices.json
    if device not in f.keys():
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute("SELECT time_col FROM devices WHERE name_on_disk = ?", (str(device),))
        time_cols = cursor.fetchone()

        conn = sqlite3.connect('database.db')  # TODO: НЕ СДЕЛАНО ЖДУ ВАНЮ!!!!!!!!!
        cursor = conn.cursor()
        cursor.execute("SELECT cols_to_draw FROM devices WHERE name_on_disk = ?", (str(device),))
        cols = cursor.fetchone()

        tech_info_dict = {'time_cols': time_cols, 'cols': cols, 'color_dict': {}}
        conn.close()

        colors = px.colors.qualitative.Alphabet
        for i in range(len(cols)):
            tech_info_dict['color_dict'][cols[i]] = colors[i]
        f[device] = tech_info_dict
        upload_json('config_devices.json', f)


    config_devices_open = load_json('config_devices.json')[device]
    time_col = config_devices_open['time_cols']
    df = df.select_dtypes(exclude=['object'])
    df[time_col] = pd.to_datetime(df[time_col], format='mixed')
    df = df[config_devices_open['cols'] + [time_col]]
    df = df.map(lambda x: x.strip() if isinstance(x, str) else x)
    name = re.split("[-_]", device)
...
@bot.message_handler(func=lambda
        message: message.text == "Просмотр данных с приборов")
...
@bot.message_handler(func=lambda message: message.text in ['Просмотр приборов по комплексам'] + list_complexes)
def choose_complexes_of_devices(message):
    pass
...
@bot.message_handler(func=lambda
        message: message.text in ['Просмотр всех приборов', 'Быстрый доступ',
                                  'Настроить быстрый доступ'] + list_devices)
def choose_device(message, list_devices_buttons=list_devices):
    if message.text in ['Просмотр всех приборов', 'Настроить быстрый доступ']:
        ...
        markup.add(*list(map(lambda x: types.KeyboardButton(x), list_devices_buttons)))
